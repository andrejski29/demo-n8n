/**
 * n8n Code node (JavaScript) — AWAY BRANCH VERSION
 *
 * "Gold Standard" normalization with robust hardening:
 * 1. Unified source (S) for all stats/coverage lookups.
 * 2. Smart denominators (Recorded -> Matches Played fallback).
 * 3. Auto-scaling for conversion rates (0.15 vs 15).
 * 4. Cards source prioritization (Cards For > Total Cards) + QA trace.
 * 5. Accurate missing-key QA.
 * 6. Robust n8n input handling.
 * 7. Identity & Side pass-through (match_id, side) for safe merging.
 * 8. Hard-default side = "away" for branch safety.
 */
...
// HARD DEFAULT for Away Branch
const side = input?.side ?? "away";
...
if (input?.homeTeam && input?.awayTeam) {
  ...
  return [{
    json: {
      ...
      side,     // Pass through (hardened default)
...
// Optional QA log for extraction
if (typeof items !== 'undefined' && items.length > 1) {
  // console.log("Note: Processing first item from n8n items array.");
}
