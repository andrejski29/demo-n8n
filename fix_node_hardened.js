/**
 * n8n Code node (JavaScript) — FINAL HARDENED VERSION
 *
 * Implements "Gold Standard" normalization with 6 key hardening fixes:
 * 1. Consistent source (S) for coverage checks.
 * 2. Denominator fallbacks (Recorded -> Matches Played).
 * 3. Conversion rate auto-scaling (>1 -> percent).
 * 4. Cards key priority (For vs Total).
 * 5. Accurate missing-key QA using unified source.
 * 6. Robust n8n input extraction.
 */
...
// ---- TWEAK 1: Coverage Block uses S (unified source) ----
...
const addShotThresholdFamily = (features, S, scope, rec, mp, needFn) => {
  const den = getDenom(rec, mp); // TWEAK 2: Use smart denominator
...
const computeTeamFeatures = (teamObj) => {
  ...
  // Unified source (S) - TWEAK 1 & 5
  const S = { ...stats, ...add };

  // QA: track keys we expect but are missing (TWEAK 5: Check against S)
  const missing = [];
  const need = (k) => {
    if (S[k] === undefined || S[k] === null || S[k] === "") missing.push(k);
  };
  const needPath = (obj, path) => { // Still useful for nested lookups if any
  ...
  // Cards raw (TWEAK 4: Ambiguity handling)
  ...
  // Goal timing shares (TWEAK 5: Checked against S via needPath if logic requires, but here simpler to just use S)
  for (const k of timingKeys15) {
    // We can use need() now since S is flat
  ...
  // Conversion metrics (TWEAK 3: Auto-scaling)
  ...
  // TWEAK 1: Pass S to coverageBlock
...
// ---- TWEAK 6: Robust Input Normalization ----
const extractTeam = (x) => {
  if (!x) return null;
...
// Case B: single team input
const team = extractTeam(input) || extractTeam(item); // Try extracting from item directly too
if (!team) {
